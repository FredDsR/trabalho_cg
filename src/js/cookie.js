...
async function main() {
  const {gl, twgl, meshProgramInfo} = initializeWorld('#cookie-canvas')

  const {obj, materials} = await loadObjAndMat('../../src/models/cookie/cookie.obj');
  ...
  function render(time) {
    ...
    const objControl = m4.addVectors(objOffset, [(state.controlX / 50),
                                                 (state.controlY / 50),
                                                 (state.zoom / 30)]);
